  ...
  <nav class="sticky top-0 left-0 right-0 h-16 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-4 z-[1000] shadow-2xl">
    ...
    <div class="flex items-center gap-4 flex-shrink-0">
      <div class="flex items-center gap-3 group cursor-pointer" @click="goHome">
        <img :src="logoUrl" alt="Vilcom Logo" class="h-10 w-auto object-contain group-hover:scale-105 transition-transform duration-300" />
      </div>
    ...
    <div class="flex items-center gap-4 flex-shrink-0">
      ...
      <div class="flex items-center gap-2 bg-slate-800/30 p-1 px-3 rounded-2xl border border-slate-800/50 hover:border-slate-700 transition-colors flex-shrink-0">
        ...
        <img :src="`https://ui-avatars.com/api/?name=${userName}&background=1e40af&color=fff`" class="size-9 rounded-xl border border-slate-700" alt="Avatar">
        
        <button @click="handleLogout" class="p-2 text-slate-400 hover:text-red-400 hover:bg-slate-800 rounded-lg transition-all flex-shrink-0" title="Secure Eject">
          <Power class="size-4" />
        </button>
      </div>
    </div>
