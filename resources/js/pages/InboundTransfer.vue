<template>
  <div class="p-6 bg-gray-100 min-h-screen">
    <div class="max-w-3xl mx-auto">
      ...
      <div v-if="loading" class="bg-white p-10 rounded-xl text-center shadow-sm">
        <i class="fa fa-spinner fa-spin text-4xl text-blue-500 mb-3"></i>
        <p class="text-gray-500">Loading your assignments...</p>
      </div>

      <div v-else-if="pendingAssignments.length === 0" class="bg-white p-10 rounded-xl text-center shadow-sm">
      ...
      <div v-else v-for="assignment in (pendingAssignments || []).filter(a => a)" :key="assignment.id" 
           class="bg-white rounded-xl shadow-md overflow-hidden mb-6 border-t-4 border-green-500">
        
        <div class="p-6">
          <div class="flex justify-between items-start mb-4">
            <div>
              <span class="text-xs font-bold text-green-600 bg-green-50 px-2 py-1 rounded uppercase">New Assignment</span>
              <h3 class="text-xl font-bold text-gray-900 mt-2">{{ assignment.asset?.model || 'Unknown Model' }}</h3>
              <p class="text-gray-500 text-sm">
                Tag: {{ assignment.asset?.asset_tag || 'N/A' }}
              </p>
              <p class="text-gray-500 text-sm" v-if="assignment.asset?.status_name">
                Status: {{ assignment.asset.status_name }}
              </p>
            </div>
          ...
          <div v-if="assignment.items && assignment.items.length" class="mb-4 p-4 bg-gray-50 rounded-lg">
            ...
            <ul class="list-disc list-inside text-sm text-gray-700 mt-2">
              <li v-for="(itm, index) in assignment.items" :key="itm?.id || index">
                <template v-if="itm">
          ...
          <div class="border-t pt-6">
            ...
            <div class="flex flex-col md:flex-row gap-3">
              <button @click="confirmAssignment(assignment.id, 'accepted')" 
              ...
              <button @click="confirmAssignment(assignment.id, 'disputed')" 
                      class="flex-1 bg-white border border-red-300 text-red-600 py-3 rounded-lg font-bold hover:bg-red-50 transition">
...
const fetchPending = async () => {
  ...
  try {
    ...
    // Laravel paginated results are in res.data.data
    pendingAssignments.value = res.data.data || res.data || [];
  } catch (err) {
...
// Logic kept as per your original file
