<template>
  <div class="max-w-7xl mx-auto space-y-10">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
      <div>
        <h1 class="text-4xl font-black text-slate-800 tracking-tight">Personal <span class="text-vilcom-blue">Consumables</span></h1>
        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-2 flex items-center gap-2">
          ...
          Allocated Supplies & Usage Tracking
        </p>
      </div>
    </div>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-gray-50/50 border-b border-gray-50">
              <th class="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Item Specification</th>
          ...
          <tbody class="divide-y divide-gray-50">
            ...
            <tr v-for="consumable in consumables" :key="consumable.id" class="group hover:bg-blue-50/30 transition-all duration-300">
              <td class="px-8 py-5">
              ...
              <td class="px-6 py-5">
                <span class="px-3 py-1 bg-slate-100 text-slate-500 rounded-lg text-[9px] font-black uppercase tracking-widest">
                  {{ consumable.category || 'Supply' }}
                </span>
              </td>
              <td class="px-6 py-5">
                <div class="font-black text-slate-700 text-sm">
                  {{ consumable.pivot.quantity }} <span class="text-[9px] text-gray-400 uppercase tracking-widest ml-1">Units</span>
                </div>
              </td>
              <td class="px-8 py-5 text-right">
                <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center justify-end gap-2">
                  ...
                  {{ formatDate(consumable.pivot.created_at) }}
                </div>
              </td>
            </tr>
            <tr v-if="!loading && consumables.length === 0">
              <td colspan="4" class="p-20 text-center text-gray-400 font-bold uppercase text-[10px] tracking-widest">
                No consumables currently assigned to your profile.
              </td>
            </tr>
          </tbody>
        </table>
...
import { Droplets, Clock, ChevronLeft, ChevronRight, Search } from 'lucide-vue-next';
