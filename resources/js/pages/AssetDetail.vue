...
const enableEditMode = async () => {
  ...
  schemaFields.forEach(field => {
    form.custom_attributes[field.key] = existingAttributes[field.key] ?? '';
  });
...
const submitAssignment = async () => {
  ...
  try {
    ...
    alert(assignmentForm.direct ? 'Asset assigned successfully.' : 'Asset assignment initiated.');
    fetchAsset(); 
  } catch (error) {
...
const printBarcode = async () => {
  ...
  try {
    ...
    alert('Barcode image could not be reached. Check your network or server route.');
    return;
  }
    ...
    <div v-else-if="asset" :class="['grid grid-cols-1 gap-10', isAdmin ? 'lg:grid-cols-4' : '']">
      ...
      <div :class="[isAdmin ? 'lg:col-span-3' : 'lg:col-span-4', 'space-y-10']">
        <div class="bg-white rounded-[3rem] shadow-sm border border-gray-100 overflow-hidden group hover:shadow-xl transition-all duration-500">
          <div class="p-10 border-b border-gray-50 flex justify-between items-end">
            <div>
          ...
          <div class="p-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            <div class="space-y-2">
        ...
        <div v-if="hasCategoryFields" class="bg-white rounded-[3rem] p-12 shadow-sm border border-gray-100 space-y-8">
